...
const updateDataBanding = (request, response) => {
    var{id_user, nilai_rata, tingkat_sertif, fakultas} = request.body
    pool.query('update tbl_banding set niali_rata = $2, tingkat_sertif  = $3 where  id_user = $1',[id_user, nilai_rata,tingkat_sertif],(error, results)=>{
        if(error){
            throw error
        }
        response.json({
